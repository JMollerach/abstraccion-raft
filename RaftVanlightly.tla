...
AlgunaVezMatchIndexNoEsCero == \E i \in Server : (
    /\ state[i] = Leader
    /\ LET \* The set of servers that agree up through index.
           ...
           agreeIndexes == {index \in 1..Len(log[i]) :
                                Agree(index) /= {}}        
        IN agreeIndexes /= {}
        \*/\ Print(agreeIndexes, agreeIndexes /= {})
        \* /\ newCommitIndex > 0
        )
        
AlgunaVezSeCommiteaAlgoEn2Servidores == \E i, j \in Server : 
    /\ i /= j
    /\ commitIndex[i] /= 0
    /\ commitIndex[j] /= 0

\*Server i has enough votes to BecomeLeader.
VotesGrantedInQuorum(i) == votesGranted[i] \in Quorum

CandidateWithGrantedQuorumBecomesLeader == [][\lnot(
            \E i \in Server :
                      /\ state[i]= Candidate 
                      /\ VotesGrantedInQuorum(i)
                      /\ BecomeLeader(i)
                      /\ state'[i]=Leader)]_vars

CandidateWithoutGrantedQuorumBecomesLeader == [][\lnot(
            \E i \in Server :
                      /\ state[i]= Candidate 
                      /\ \lnot(VotesGrantedInQuorum(i))
                      /\ BecomeLeader(i)
                      /\ state'[i]=Leader)]_vars

NonTrivialCommittedLogUpToDate == \E i \in Server :
                                  (/\ state[i] = Leader
                                  /\ Len(log[i]) > 0
